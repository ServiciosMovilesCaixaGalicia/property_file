%%%----------------------------------------------------------------------
%%% File    : property_mlex.erl
%%% Author  :  <svg@surnet.ru>
%%% Purpose : Free Radius test grammar
%%% Created : 29 Jul 2002 by  <svg@surnet.ru>
%%%----------------------------------------------------------------------
%%%
%%% $Id: property_mlex.erl,v 1.1.1.1 2002/10/04 00:00:00 svg Exp $
%%%
%%% $Log: property_mlex.erl,v $
  ...
  Grammar = 
    mlex:grammar(
      [{decl_end,       decl_end(),       fun yeec_token/3},
       ...
       {false_kw,       false_kw(),       fun yeec_token/3},
       {assign_op,      assign_op(),      fun yeec_token/3},
       {operator,       operator(),       fun yeec_token_clike/3},
       {r_brace,        r_brace(),        fun yeec_token/3},
...
%% Format funs
%%
yeec_token_apache(Class=atom_string, Line, Str="Include") ->
  {operator, Line, "INCLUDE"};
yeec_token_apache(Class, Line, Str) ->
  yeec_token(Class, Line, Str).

yeec_token_clike(Class=operator, Line, Str) ->
  {operator, Line, unquote_chars(tl(Str))};
yeec_token_clike(Class, Line, Str) ->
  yeec_token(Class, Line, Str).

yeec_token(Class=ipv4_address, Line, Str) ->
  case parse_ipv4_address(Str) of
    Error={error, _} ->
      Error;
    IP ->
      {Class, Line, IP}
  end;
yeec_token(Class=ipv4_netmask, Line, Str) ->
  case parse_ipv4_netmask(Str) of
    Error={error, _} ->
      Error;
    Mask ->
      {Class, Line, Mask}
...
unquote_chars([$\\, C3, C2, C1|Rest], Acc) when C3 =< $7,
						...
						C2 >= $0,
						C1 >= $0 ->
  unquote_chars(Rest, [((C3-48)*8 + (C2-48))*8 +(C1-48)|Acc]);
...
parse_ipv4_netmask(Str) ->
  case catch
    begin
      [StrIP, StrMask] = string:tokens(Str, "/"),
      {ok, IP} = inet:getaddr(StrIP, inet),
      {ok, Mask} =
	case inet:getaddr(StrMask, inet) of
	  {ok, GoodIP} ->
	    ipv4_to_netmask(GoodIP);
	  _ ->
	    int_to_ipv4_netmask(StrMask)
	end,
      {ok, {IP, Mask}}
    end of
    {ok, Result} ->
      Result;
    Error ->
      {error, {bad_netmask, Str}}
  end.

ipv4_to_netmask({A4, A3, A2, A1}) ->
  {ok, (A4 bsl 24) + (A3 bsl 16) + (A2 bsl 8) + A1}.

int_to_ipv4_netmask(StrMask) when list(StrMask) ->
  int_to_ipv4_netmask(list_to_integer(StrMask));
int_to_ipv4_netmask(Num) when integer(Num),
			      Num =< 32,
			      Num >= 0 ->
  {ok, (16#ffffffff bsl (32-Num)) band 16#ffffffff}.
